...
import {
  aluminiosCategories,
  getAluminiosCategoryBySlug,
  type AluminiosProduct,
  type AluminiosCategory,
} from "../data/aluminiosCategories";
...
function ProductCard({
  ...
  product: AluminiosProduct;
  cat: AluminiosCategory;
  index: number;
}) {
  return (
    <motion.div
...
function OtherCategories({ currentSlug }: { currentSlug: string }) {
  const others = aluminiosCategories.filter((c) => c.slug !== currentSlug);
  return (
...
export default function AluminiosSubPage() {
  const params = useParams<{ slug: string }>();
  const cat = getAluminiosCategoryBySlug(params.slug ?? "");
  ...
  return (
    <motion.div {...pageTransition}>
      ...
      <section className="relative min-h-[62vh] flex items-end pb-12 overflow-hidden bg-[#151515]">
        {/* Cinematic background — aluminio.png matching main Aluminios page */}
      ...
      {cat.products.some((p) => p.featured && p.imageUrl) && (
        <section className="py-12 bg-[#111111]">
          <div className="max-w-6xl mx-auto px-6">
            ...
            >
              <div className="w-4 h-px bg-zinc-500/40" />
              <span className="text-[10px] font-bold tracking-[0.24em] text-zinc-500 uppercase">Proyecto destacado</span>
            </motion.div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-7 items-center">
              <div>
                ...
                <p className="text-zinc-500 text-sm leading-relaxed">
                  Relevamiento sin costo. Fabricamos a medida para tu proyecto.
                </p>
              </div>
