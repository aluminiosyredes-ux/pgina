...
import { MessageCircle, ArrowLeft, ChevronRight, ShoppingCart, ArrowRight, Check, ShieldCheck, Truck, ChevronLeft } from "lucide-react";
import { useState, useEffect, useRef } from "react";
import { WHATSAPP_NUMBER } from "../config";
import AsistenciaStrip from "../components/AsistenciaStrip";
import { getCategoryBySlug, type DomoticaProduct, type DomoticaCategory } from "../data/domoticaCategories";
...
function ProductCard({
  ...
  product: DomoticaProduct;
  cat: DomoticaCategory;
  index: number;
}) {
  return (
    <motion.div
        ...
        <div className="flex flex-col gap-1.5 pt-3 border-t border-white/[0.04]">
          ...
          {product.mlLink && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-1.5 px-4 py-2 bg-yellow-400/8 border border-yellow-500/18 text-yellow-400 text-[9px] font-bold tracking-[0.18em] uppercase rounded-xl hover:bg-yellow-400/14 hover:border-yellow-500/30 transition-all duration-300"
            >
...
function FeaturedProductCard({
  ...
  return (
    <motion.div
      ...
      style={{ boxShadow: "0 0 60px rgba(0,0,0,0.6), 0 0 30px rgba(139,92,246,0.04)" }}
    >
      {/* Top accent */}
      <div className={`h-[2px] bg-gradient-to-r from-transparent ${cat.color.replace("text-", "via-").replace("400", "400/60")} to-transparent`} />

      <div className="grid grid-cols-1 md:grid-cols-[380px_1fr]">

        {/* ── Image panel ── */}
        <div className="relative flex items-center justify-center bg-[#222222] border-b md:border-b-0 md:border-r border-white/[0.05] min-h-[260px] md:min-h-[340px] overflow-hidden">
          {/* Subtle glow behind image */}
          <div className={`absolute inset-0 ${cat.glowBg} opacity-60`} />
          {/* Dot grid */}
          <div
        ...
        <div className="flex flex-col p-7 md:p-8 gap-5">
          ...
          <div className="flex flex-wrap gap-3">
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-emerald-500/18 bg-emerald-500/6">
              ...
              <span className="text-[9px] font-bold tracking-[0.18em] uppercase text-emerald-400">Compra Segura</span>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2.5 mt-auto">
            {product.mlLink && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="group flex items-center justify-center gap-2 px-6 py-3 bg-amber-500 hover:bg-amber-400 text-black font-black text-[10px] tracking-[0.2em] uppercase rounded-xl transition-all duration-300 shadow-lg shadow-amber-500/15 hover:shadow-amber-500/25 hover:scale-[1.02]"
              >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 px-6 py-3 bg-[#25D366]/10 border border-[#25D366]/22 text-[#25D366] font-bold text-[10px] tracking-[0.18em] uppercase rounded-xl hover:bg-[#25D366]/18 hover:border-[#25D366]/35 transition-all duration-300"
            >
...
const INTERRUPTOR_IMAGES = [
  "/images/interruptorinteligente1.png",
  "/images/interruptorinteligente2.png",
  "/images/interruptorinteligente3.png",
  "/images/interruptorinteligente4.png",
  "/images/interruptorinteligente5.png",
  "/images/interruptorinteligente6.png",
];

const MOTORCORTINA_IMAGES = [
  "/images/motorcortina1.png",
  "/images/motorcortina2.png",
  "/images/motorcortina3.png",
];

const CERRADURA_IMAGES = [
  "/images/cerradurainteligente1.png",
  "/images/cerradurainteligente2.png",
  "/images/cerradurainteligente3.png",
  "/images/cerradurainteligente4.png",
];

const CONTROLVOZ_IMAGES = [
  "/images/controlvoz1.png",
  "/images/controlvoz2.png",
  "/images/controlvoz3.png",
  "/images/controlvoz4.png",
];

const AUTOMATIZACION_IMAGES = [
  "/images/inteligente1.png",
  "/images/inteligente2.png",
  "/images/inteligente3.png",
  "/images/inteligente4.png",
];
...
function ImageSlider({ config }: { config: SliderConfig }) {
  ...
  const next = () => goTo((current + 1) % config.images.length, 1);
  const prev = () => goTo((current - 1 + config.images.length) % config.images.length, -1);
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            <div className={`w-4 h-px ${config.accentColor}`} />
            <span className="text-[10px] font-bold tracking-[0.24em] text-zinc-500 uppercase">Galería de productos</span>
          </div>
        ...
        <div className="relative rounded-2xl overflow-hidden border border-white/[0.06] bg-[#111] select-none" style={{ height: 480 }}>
          <AnimatePresence mode="wait" custom={dir}>
            ...
            >
              <img
                ...
                alt={`${config.altPrefix} ${current + 1}`}
          ...
          <div className="absolute bottom-4 right-4 px-2.5 py-1 rounded-full text-[10px] font-bold text-white/50 z-10"
            style={{ background: "rgba(0,0,0,0.5)", border: "1px solid rgba(255,255,255,0.08)" }}>
            {current + 1} / {config.images.length}
          </div>
            ...
            {config.images.map((_, i) => (
              <button
                ...
                style={{
                  ...
                  height: 6,
                  background: i === current ? config.dotActiveColor : "rgba(255,255,255,0.2)",
                }}
              />
            ))}
          </div>
...
const SLIDER_CONFIGS: Record<string, SliderConfig> = {
  iluminacion: {
    images: INTERRUPTOR_IMAGES,
    title: "Interruptores y enchufes inteligentes.",
    description: "Reemplaza tus interruptores y enchufes convencionales por soluciones WiFi controladas desde tu smartphone. Sin obras mayores, instalación simple y compatible con Alexa y Google Home.",
    accentColor: "bg-yellow-500/40",
    dotActiveColor: "rgba(234,179,8,0.8)",
    altPrefix: "Interruptor inteligente",
  },
  cortinas: {
    images: MOTORCORTINA_IMAGES,
    title: "Motores para cortinas y persianas.",
    description: "Automatiza tus cortinas roller y persianas con motores silenciosos WiFi. Control desde app, por voz o interruptor táctil — sin necesidad de cambiar tus cortinas actuales.",
    accentColor: "bg-blue-500/40",
    dotActiveColor: "rgba(96,165,250,0.8)",
    altPrefix: "Motor cortina",
  },
  "control-voz": {
    images: CONTROLVOZ_IMAGES,
    title: "Control por voz con Alexa, Google Home y HomeKit.",
    description: "Integramos tu hogar con los principales asistentes de voz del mercado. Configuración profesional de rutinas, escenas y automatizaciones por comando de voz.",
    accentColor: "bg-purple-500/40",
    dotActiveColor: "rgba(168,85,247,0.8)",
    altPrefix: "Control por voz",
  },
  automatizacion: {
    images: AUTOMATIZACION_IMAGES,
    title: "Automatización del hogar.",
    description: "Crea escenas y rutinas que se activan solas: luces, temperatura, cortinas y seguridad coordinados en un solo ecosistema. Tu casa trabaja por ti sin que toques nada.",
    accentColor: "bg-emerald-500/40",
    dotActiveColor: "rgba(52,211,153,0.8)",
    altPrefix: "Hogar inteligente",
  },
  "acceso-inteligente": {
    images: CERRADURA_IMAGES,
    title: "Cerraduras y control de acceso inteligente.",
    description: "Cerraduras electrónicas, videoporteros y sistemas RFID que eliminan las llaves físicas. Controlá quién entra y cuándo, desde cualquier parte del mundo con tu smartphone.",
    accentColor: "bg-amber-500/40",
    dotActiveColor: "rgba(245,158,11,0.8)",
    altPrefix: "Acceso inteligente",
  },
};

// ─── PAGE ──────────────────────────────────────────────────────────────────────

export default function DomoticaSubPage() {
  ...
  if (!cat) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-zinc-500 gap-4">
        ...
        <Link href="/domotica" className="text-xs font-bold tracking-widest uppercase text-amber-400 hover:text-amber-300">
          ← Volver a Domótica
        </Link>
      </div>
    );
  ...
  return (
    <motion.div {...pageTransition}>
      ...
      <section className="relative min-h-[62vh] flex items-end pb-12 overflow-hidden bg-[#151515]">
        {/* Cinematic background — domo.png matching main Domótica page */}
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 w-full pt-20">
          ...
          >
            ...
            <h1 className="text-5xl md:text-7xl font-black text-white tracking-tighter leading-[0.9] mb-5">
              {cat.title.split(" ").slice(0, -1).join(" ")}<br />
              <span className="text-zinc-500">{cat.title.split(" ").slice(-1)[0]}.</span>
            </h1>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              >
                <ArrowLeft size={13} /> Ver todas las categorías
              </Link>
            </div>
          </motion.div>
        </div>
      ...
      {/* ── IMAGE SLIDER ── */}
      {SLIDER_CONFIGS[cat.slug] && <ImageSlider config={SLIDER_CONFIGS[cat.slug]} />}

      {/* ── FEATURED PRODUCTS ── */}
      {!SLIDER_CONFIGS[cat.slug] && cat.products.some((p) => p.featured && p.imageUrl) && (
        <section className="py-12 bg-[#111111]">
          <div className="max-w-6xl mx-auto px-6">
            ...
            >
              <div className={`w-4 h-px ${cat.color.replace("text-", "bg-").replace("400", "500/50")}`} />
              <span className="text-[10px] font-bold tracking-[0.24em] text-zinc-500 uppercase">
                Producto destacado
              </span>
            </motion.div>
      ...
      {/* ── PRODUCTS ── */}
      {!SLIDER_CONFIGS[cat.slug] && cat.products.filter((p) => !p.featured).length > 0 && (
          ...
          >
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-[10px] font-bold tracking-[0.22em] text-zinc-500 uppercase">Servicios y soluciones</span>
              </div>
              <h2 className="text-2xl md:text-3xl font-black text-white tracking-tight">
                {cat.products.filter((p) => !p.featured).length} soluciones disponibles
                <span className="text-zinc-600 font-light"> en esta categoría.</span>
              </h2>
            </div>
            <a
              href={wa(cat.waMsg)}
      ...
      <section className="py-10 bg-[#151515] border-t border-white/[0.04]">
        <div className="max-w-6xl mx-auto px-6">
          ...
          >
            ...
            <h3 className="text-xl font-black text-white tracking-tight">Otras categorías de domótica.</h3>
          </motion.div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-7 items-center">
              <div>
                ...
                <p className="text-zinc-500 text-sm leading-relaxed">
                  Asesoramiento sin costo. Diseñamos la solución ideal para tu espacio.
                </p>
              </div>
...
function OtherCategories({ currentSlug }: { currentSlug: string }) {
  const others = domoticaCategories.filter((c) => c.slug !== currentSlug);
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2.5">
      {others.map((c, i) => {
        ...
        return (
          ...
          >
            <Link
              href={`/domotica/${c.slug}`}
              className={`group flex flex-col items-center gap-2.5 p-4 rounded-2xl border ${c.border} ${c.hoverBorder} bg-[#1c1c1c] text-center transition-all duration-300`}
            >
