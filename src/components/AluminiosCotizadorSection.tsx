...
import {
  Maximize2, Wind, LayoutTemplate, ArrowLeftRight,
  Building2, Sparkles,
...
const TYPES = [
  {
    id: "ventanales",
    icon: Maximize2,
    name: "Ventanales de Aluminio",
    desc: "Piso a techo. Máxima luminosidad y elegancia arquitectónica.",
    tag: "Más solicitado",
    color: "text-zinc-300",
  ...
  },
  {
    ...
    desc: "Protección total contra viento y lluvia sin perder la vista.",
    tag: "Outdoor",
    color: "text-sky-400",
  ...
  },
  {
    ...
    desc: "Ambientes modulares y elegantes en aluminio y vidrio.",
    tag: "Diseño interior",
    color: "text-violet-400",
    accent: "border-violet-400/35 bg-violet-400/8",
  ...
  },
  {
    ...
    desc: "Solución diseñada a medida según tu espacio y necesidades.",
    tag: "A medida",
    color: "text-emerald-400",
    accent: "border-emerald-400/35 bg-emerald-400/8",
...
type AluminioType = Omit<(typeof TYPES)[number], "priceMin" | "priceMax"> & {
  icon: LucideIcon;
  priceMin: number;
  priceMax: number;
};
...
function Steps({ current }: { current: 1 | 2 | 3 }) {
  return (
    <div className="flex items-center gap-0 mb-8">
      {(["Proyecto", "Medidas", "Resumen"] as const).map((label, i) => {
        ...
        return (
          <div key={label} className="flex items-center">
            <div className="flex flex-col items-center gap-1">
              <div
                className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black transition-all duration-400
              ...
              <span
                className={`text-[8px] font-bold tracking-[0.16em] uppercase transition-colors duration-300
            ...
            {i < 2 && (
              <div
                className={`w-12 sm:w-20 h-px mx-2 mb-3.5 transition-all duration-400 ${
                  current > num ? "bg-amber-500/50" : "bg-zinc-800"
                }`}
              />
            )}
          </div>
  ...
  return (
    <div className="flex flex-col gap-5">
      <h3 className="text-xl font-black text-white tracking-tight">
        ¿Qué tipo de proyecto<span className="text-zinc-500"> necesitás?</span>
      </h3>
            ...
            >
              ...
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5 ${
                  on ? "bg-black/20" : "bg-zinc-800"
                }`}
              >
                <Icon size={15} className={on ? ct.color : "text-zinc-500"} />
              </div>
              <div className="flex-1 min-w-0">
                <p className={`text-[12px] font-black leading-tight mb-0.5 ${on ? "text-white" : "text-zinc-200"}`}>
                  {ct.name}
                </p>
...
function Step2({
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="rounded-xl border border-white/[0.04] bg-[#1e1e1e] p-4 mb-1">
        <p className="text-[9px] font-bold tracking-[0.18em] uppercase text-zinc-600 mb-0.5">Proyecto seleccionado</p>
      ...
      <AnimatePresence>
        {m2 !== null && (
          ...
          >
            <span className="text-[10px] font-bold tracking-widest uppercase text-zinc-500">Superficie total</span>
            <span className="text-lg font-black text-white">
              {fmtM(m2)} <span className="text-sm text-zinc-500 font-medium">m²</span>
            </span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex items-center justify-between">
        <button
          onClick={onBack}
          className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl border border-zinc-800 text-zinc-500 font-bold text-[9px] tracking-widest uppercase hover:border-zinc-700 hover:text-zinc-300 transition-all"
        >
...
function Step3({
  ...
  const waText =
    `Hola! Realicé una cotización de aluminio:\n` +
  ...
  return (
    <div className="flex flex-col gap-5">
        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <div
                key={item.l}
                className="flex flex-col items-center gap-0.5 py-2.5 rounded-lg border border-white/[0.05] bg-black/20"
              >
          ...
          <div className="rounded-xl border border-amber-500/12 bg-amber-500/[0.03] p-4">
            ...
            <p className="text-[9px] text-zinc-600 mt-1.5">CLP · Mano de obra no incluida</p>
          </div>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="flex flex-col gap-1">
          <p className="text-[10px] text-zinc-300 font-bold leading-relaxed">
            Los valores entregados son aproximados.
          </p>
          <p className="text-[10px] text-zinc-500 leading-relaxed">
            Para un presupuesto final es necesaria una visita técnica o videollamada. La instalación se coordina directamente con la empresa.
          </p>
        </div>
      </motion.div>
      ...
      <div className="flex items-center justify-between pt-1">
        ...
        <button
          onClick={onReset}
          className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl border border-zinc-800/50 text-zinc-700 font-bold text-[9px] tracking-widest uppercase hover:text-zinc-500 transition-all"
        >
...
export default function AluminiosCotizadorSection() {
  ...
  const pricedTypes: AluminioType[] = TYPES.map((t) => {
    const p = prices.aluminios.find((r) => r.id === t.id);
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <h2 className="text-3xl md:text-4xl font-black text-white tracking-tight">
            Cotizá tu proyecto
            <span className="text-zinc-600 font-light"> de aluminio.</span>
          </h2>
